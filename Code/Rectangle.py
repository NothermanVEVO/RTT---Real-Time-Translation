...
def find_connected_groups(rectangles: List[Rectangle], max_distance: float) -> List[Set[int]]:
    ...
    def dfs(i, group):
        visited[i] = True
        group.add(i)
        for j in range(n):                                                                                 # Juntar soh os retangulos com tamanho parecido
            if not visited[j] and distance_between_rects(rectangles[i], rectangles[j]) <= max_distance and abs(rectangles[i].height - rectangles[j].height) <= distance_gap:
                dfs(j, group)
    for i in range(n):
        if not visited[i]:
            group = set()
...
#     return merged
def find_and_merge_close_rectangles(rectangles: List[Rectangle], max_distance: float) -> tuple[List[Rectangle], List[int], List[float]]:
    groups = find_connected_groups(rectangles, max_distance)
    ...
    for group in groups:
        ...
        merged = merge_rectangles_list(group_rects)
        merged_rectangles.append(merged)
        counts.append(len(group))
        avg_height = sum(r.height for r in group_rects) / len(group_rects)
        avg_heights.append(avg_height)
    
    return merged_rectangles, counts, avg_heights
